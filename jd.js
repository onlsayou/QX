...
var merge = {
  ...
  JDBtupdate:  {success:0,fail:0,bean:0,steel:0,notify:'',Edu:'',Tedu:0,Edu0:0,key:0}
}
...
function notify() {
  return new Promise(resolve => {
    try {
      ...
      var Edu = merge.JDBtupdate.Tedu ? merge.JDBtupdate.Tedu + "元白条" : ""
      var bsc = beans ? "\n" : Steel ? "\n" : Cash ? "\n" : "获取失败\n"
   ...
   //添加京东白条
function JingDongBtupdate(s) {
  return new Promise(resolve => { setTimeout(() => {
    const JDkey1 = {
      url: 'https://ms.jr.jd.com/gw/generic/bt/h5/m/getRiseLimitItems',
      headers: {
        Cookie: KEY,
        Referer: "https://cfm.jd.com/jdbt/manage/pages/home.html?channelName=00007",
      },
      body : "reqData=%7B%22clientType%22:%22ios%22,%22clientVersion%22:%2213.3.1%22,%22eid%22:%22PL34MDC7DLTFUPKCWWEAD6SOVNLCWQDWGQFNVDB7WF5AKZSSXLZ63CGLIX7HQAQLXJ4UDCQ7BBPMHKOACJBFLT6WKM%22,%22fp%22:%2290ac7ea99a31893c82dc4a280022f835%22,%22riskDeviceInfo%22:%7B%22macAddress%22:%2202:00:00:00:00:00%22,%22channelInfo%22:%22appstore%22,%22IPAddress1%22:%22192.168.1.102%22,%22OpenUDID%22:%2273a4df6f5678be4a8291569ab7ad3f1c8a36a566%22,%22clientVersion%22:%225.3.40%22,%22terminalType%22:%2202%22,%22osVersion%22:%2213.3.1%22,%22appId%22:%22com.jd.jinrong%22,%22deviceType%22:%22iPhone10,3%22,%22networkType%22:%22WIFI%22,%22startNo%22:1278,%22UUID%22:%22%22,%22IPAddress%22:%22%22,%22deviceId%22:%22DC540F00-6F25-4FBA-9699-84E022069632%22,%22IDFA%22:%229E2F9179-8EA2-4955-B9DF-7E89E35224C9%22,%22resolution%22:%221125*2436%22,%22osPlatform%22:%22iOS%22%7D%7D"
    };
    $nobyda.post(JDkey1, function(error, response, data) {
      try {
        if (error) {
          merge.JDBtupdate.notify = "京东金融-白条: 登录接口请求失败 ‼️‼️"
          merge.JDBtupdate.fail = 1
          resolve('done')
        } else {
          if (data.match(/\"uniqueCode\":\"[0-9]{4,}\"/)) {
            
            const cc = JSON.parse(data)
            merge.JDBtupdate.key = cc.resultData.raiseItemList[1].uniqueCode
            merge.JDBtupdate.Edu0 = cc.resultData.raiseItemList[1].changeLimit
            if (log) console.log("京东金融-白条登录成功, KEY获取成功: \n" + data)
            if (merge.JDBtupdate.key) {
              const JDBUrl = {
                ...
                body : 'reqData=%7B%22clientType%22:%22ios%22,%22clientVersion%22:%2213.3.1%22,%22packageId%22:%22'+merge.JDBtupdate.key+'%22%7D'
              };
              $nobyda.post(JDBUrl, function(error, response, data) {
                try {
                  if (error) {
                    merge.JDBtupdate.notify = "京东白条-提额: 接口请求失败 ‼️‼️"
                    merge.JDBtupdate.fail = 1
                  } else {
                    const c = JSON.parse(data)
                    if (data.match(/\"totalBalanceAmount\":\d+/)) {
   
                        merge.JDBtupdate.Tedu = c.resultData.totalBalanceAmount + parseInt(c.resultData.changeBalanceAmount)
                      } 
                    if (data.match(/\"error_msg\":\"成功\"/)) {
                      if (log) console.log("京东白条-提额成功 response: \n" + data)
                      if (data.match(/\"changeBalanceAmount\":\"[0-9]{1,}\"/)) {
                        merge.JDBtupdate.notify = "京东白条-提额: 成功, 提升: " + c.resultData.changeBalanceAmount + "额度 🆙"
                        merge.JDBtupdate.success = 1
                        merge.JDBtupdate.Edu = c.resultData.changeBalanceAmount
                      } 
                 
                  }
                    else{
                    
                     if (log) console.log("京东白条-提额失败 response: \n" + data)
                      merge.JDBtupdate.notify = "京东白条-提额: 失败, 原因:已提过 ☑️\n         ↘️↘️↘️ 明日可提-额度："+merge.JDBtupdate.Edu0+" 元"
                        merge.JDBtupdate.fail= 1
                    }
                  resolve('done')
                } 
                   }catch (eor) {
                  $nobyda.notify("京东白条-提额" + eor.name + "‼️", JSON.stringify(eor), eor.message)
                  resolve('done')
                }
              })
            }
          }else {
            if (log) console.log("京东白条-提额登录失败response: \n" + data)
            if (data.match(/(未登录|\"101\")/)) {
              merge.JDBtupdate.notify = "京东白条-提额: 失败, 原因: Cookie失效‼️"
              merge.JDBtupdate.fail = 1
            }
          }
        }
        ...
        $nobyda.notify("京东白条-提额登录" + eor.name + "‼️", JSON.stringify(eor), eor.message)
        resolve('done')
      }
    })}, s)
  });
